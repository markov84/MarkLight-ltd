...
export default function Admin() {
  // Drag-to-scroll refs and state (MUST be inside the component)
  ...
  const [formData, setFormData] = useState({
    ...
    newCategory: "",
    newSubcategory: "",
    // removed localImageFile, localImagePreview
  });
  ...
  const fetchProducts = async (pageNum = 1) => {
    try {
      setLoading(true);
      const response = await axios.get(`${API}/admin/products?page=${pageNum}&limit=20`, {
        withCredentials: true
      });
      setProducts(response.data.products || response.data);
      setTotalPages(response.data.totalPages || 1);
      setPage(Number(response.data.currentPage) || 1);
    } catch (err) {
  ...
  const handleInputChange = (e) => {
    ...
    } else if (name === 'localImageFile') {
      const file = files && files[0];
      setFormData(prev => ({
        ...prev,
        localImageFile: file,
        localImagePreview: file ? URL.createObjectURL(file) : ""
      }));
    } else {
      setFormData(prev => ({
        ...prev,
      ...
      if (showNewSubcategory && formData.newSubcategory && categoryId) {
        try {
          ...
          setError(
            ...
            "Грешка при създаването на подкатегория. Проверете дали името вече не съществува."
          );
          setSubmitting(false);
          return;
        }
  ...
  <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 w-full mx-auto">
  ...
  <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 w-full mx-auto">
      ...
      {/* Product Form */}
  <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 w-full mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            <input
              ...
              onChange={handleInputChange}
              className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              placeholder="https://example.com/image.jpg (основна снимка)"
            />
            <input
              type="url"
              name="images"
              ...
              onChange={handleInputChange}
              className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white mt-2"
              placeholder="https://example.com/image1.jpg (допълнителна снимка по URL)"
            />
            {/* File input for local image */}
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mt-2">Качи допълнителна снимка</label>
            {/* Removed file upload field and preview */}
          </div>
      ...
      {/* Products List */}
  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 w-full mx-auto">
        ...
        {loading ? (
          ...
          <div className="relative">
            {/* Scroll Arrows */}
            <button
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full p-2 shadow hover:bg-blue-500 hover:text-white transition disabled:opacity-30"
              style={{ left: 4 }}
              onClick={() => {
            ...
            <button
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full p-2 shadow hover:bg-blue-500 hover:text-white transition disabled:opacity-30"
              style={{ right: 4 }}
              onClick={() => {
                if (tableScrollRef.current) tableScrollRef.current.scrollBy({ left: 200, behavior: 'smooth' });
              }}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        {[ 
                          ...
                          // If image is a relative path (starts with /uploads), prepend API base URL
                          let src = img;
          ...
          <div className="flex justify-center items-center gap-2 py-4">
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                key={i}
                className={`px-3 py-1 rounded font-semibold mx-1 ${page === i + 1 ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200'}`}
                onClick={() => setPage(i + 1)}
              >
                {i + 1}
              </button>
